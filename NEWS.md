...
- When using R 4.5.0 or greater, plots saved via the `pdf()` device are now
  ...
  default (#64).
  - This PR refactors some of the changes implemented in (#59) to better 
    harmonize with the `CairoPDF()` device implementation.

- Cairo-based pdf output can be generated by passing `dev = "CairoPDF"`, which
